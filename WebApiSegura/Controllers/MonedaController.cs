    ...
    public class MonedaController : ApiController
    {
        ...
        public IHttpActionResult Ingresar(Moneda moneda)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"INSERT INTO Moneda (Descripcion, Estado) 
                                                                VALUES 
        ...
        public IHttpActionResult Actualizar(Moneda moneda)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"UPDATE Moneda set 
        ...
        public IHttpActionResult Eliminar(int id)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"DELETE Moneda 
                                                            WHERE Codigo = @Codigo", sqlConnection);
