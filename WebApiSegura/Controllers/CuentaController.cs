    ...
    public class CuentaController : ApiController
    {
        ...
        public IHttpActionResult GetId(int id)
        {
            ...
            try
            {
                ...
                {
                    SqlCommand sqlCommand = new SqlCommand(@"SELECT Codigo, CodigoUsuario, CodigoMoneda, Descripcion, IBAN, Saldo, Estado
                                                            FROM Cuenta Where Codigo= @Codigo", sqlConnection);
    ...
    public IHttpActionResult Ingresar(Cuenta cuenta)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"INSERT INTO Cuenta (CodigoUsuario, CodigoMoneda, Descripcion, IBAN, Saldo, Estado) 
                                                                VALUES 
                                                                (@CodigoUsuario, @CodigoMoneda, @Descripcion, @IBAN, @Saldo, @Estado)", sqlConnection);
        ...
        public IHttpActionResult Actualizar(Cuenta cuenta)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"UPDATE Cuenta set CodigoUsuario = @CodigoUsuario,
        ...
        public IHttpActionResult Eliminar(int id)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"DELETE Cuenta 
                                                            WHERE Codigo = @Codigo", sqlConnection);
